...
def _generate_random_room_parameters():
    ...
    mic_positions_relative = random_microphone_array_position(mic_space_size, hyp.N_RASPI)
    mic_positions = mic_positions_relative + np.array([[mic_min_dist_wall]] * len(room_dim))
    return {"room_dim": room_dim, "max_order": max_order,
            "source_pos": source_pos, "mic_positions": mic_positions, "fs": hyp.DownsampleRate}


def make_new_pra_room():
    params = _generate_random_room_parameters()
    materials = pra.make_materials(**hyp.RoomMaterial)
    room = pra.ShoeBox(params["room_dim"], fs=params["fs"], materials=materials,
...
class RandomScale(nn.Module):  # (Implementation remains the same as rpi_ae_preprocessor_v2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scales = (self.scale_min + (self.scale_max - self.scale_min) * torch.rand(batch_size, num_mics, 1,
                                                                                  device=x.device))
        return x * scales
...
class RPIAE_Train_DataPreprocessor(nn.Module):
    def __init__(self):
        ...
        self.rand_rir_augmenter = AugmentedDataPreprocessor(
            n_mic=hyp.N_RASPI,
            reset_interval=hyp.ResetRoomInterval,  # This will be BatchSize as per discussion
            device=opt.Device
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not (x.dim() == 3 and x.shape[0] == 1 and x.shape[1] == 1):
            raise ValueError(
                f"RPIAE_Train_DataPreprocessor expects input from __getitem__ as [1,1,T] or [1,T], received {x.shape}")
        ...
        x_fixed_length = x_scaled[..., 0 : hyp.DownsampleRate * hyp.AudioDuration]
...
class RPIAE_Eval_DataPreprocessor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not (x.dim() == 3 and x.shape[0] == 1 and x.shape[1] == 1):
            raise ValueError(
                f"RPIAE_Eval_DataPreprocessor expects input from __getitem__ as [1,1,T] or [1,T], received {x.shape}"
            )
